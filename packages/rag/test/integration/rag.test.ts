...
test('RAG Integration: Document ingestion and processing', { timeout: 60000 }, async (t) => {
  ...
  const noChunkResult = await rag!.ingest(simpleDoc as unknown as string[], modelName, {
    chunk: false
  })
...
test('RAG Integration: Document deletion and cleanup', { timeout: 30000 }, async (t) => {
  ...
  const saveResult = await rag!.ingest(docsToDelete as unknown as string[], modelName, {
    chunk: false
  })
...
test('RAG Integration: Error handling and edge cases', { timeout: 30000 }, async (t) => {
  ...
  try {
    await rag!.ingest(duplicateDocs as unknown as string[], modelName, { chunk: false })
  ...
  const emptyResult = await rag!.ingest(emptyContentDocs as unknown as string[], modelName, {
    chunk: false
  })
  t.is(
  ...
  const largeResult = await rag!.ingest(largeDoc as unknown as string[], modelName, {
    chunk: false
  })
