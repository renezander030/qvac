# ggml-speech: tetherto/qvac-ext-ggml@speech. This pin fixes the Vulkan matmul
# src0 binding for strided views (PR #53). ggml_vk_mul_mat_q_f16 and
# ggml_vk_mul_mat_id_q_f16 sized the src0 subbuffer from the view's elements
# counted as if they were packed, but a src0 read in place is walked by the
# shader with the real channel stride, so every channel past the first was
# addressed beyond the end of the binding. What a driver returns there is
# undefined, which is why desktop never noticed and this device did. Sizing from
# ggml_nbytes(src0) spans nb[] and equals the old value for a genuinely packed
# tensor, so it only ever widens a range and cannot change a working path.
# test-backend-ops -o MUL_MAT -b Vulkan0 -p m_alloc goes 0/8 -> 8/8 on a Samsung
# Xclipse 920. The ACE-Step LM's KV window is exactly this shape: greedy decode
# there went from a degenerate repeated token to a stable trajectory in the same
# range the CPU reference occupies.
#
# The same pin defaults prefer_host_memory on when the device reports unified
# memory. Device-local-first buys a UMA adapter nothing but a staging copy, so
# ACE-Step q8_0 with every stage on the GPU goes 25114 -> ~20000 ms total and the
# DiT stage 10792 -> 7804 ms on that device, with a byte-identical render.
#
# On OpenCL it adds ACE-Step support for Adreno and makes 1-D convolution fast
# (PR #52): snake and col2im_1d kernels, bf16 get_rows, an N-sliced xmem GEMM so
# wide-N matmuls stop falling back, and an im2col rewrite. im2col had adjacent
# lanes storing CHW elements apart -- 1792 bytes for this model, so every 2-byte
# store owned a cache line -- and did 64-bit division twice per element, which
# Adreno emulates in software; it measured 81% of the ACE-Step VAE decode on an
# Adreno 740. One workgroup per output position now walks that position's
# contiguous CHW columns in 32-bit index math: im2col 34944 -> 205 ms, whole VAE
# decode 43064 -> 8376 ms of GPU exec. im2col is a pure gather, so this is
# bit-exact, and it helps every conv1d/conv2d model on OpenCL, not just ACE-Step.
#
# Carries Metal CPY from the k-quants and the Metal im2col threadgroup sizing
# (PR #51), the unaligned-M Adreno im2col fusion (PR #50), the Adreno OpenCL
