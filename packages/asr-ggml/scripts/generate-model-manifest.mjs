...
async function main () {
  ...
  // `coremlSidecars` zips stage under models/coreml/ (see
  // stage-integration-models.mjs), so pin them from there.
  const entries = [
    ...Object.entries(manifest.models || {}).map(([name, entry]) => [name, entry, join(args.models, name)]),
    ...Object.entries(manifest.coremlSidecars || {}).map(([name, entry]) => [name, entry, join(args.models, 'coreml', name)])
  ]
