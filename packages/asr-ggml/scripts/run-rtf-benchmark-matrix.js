...
//   parakeet: { engine, modelType, quant?, useGPU, backendHint?, coreml?,
//               maxThreads?, numRuns?, numWarmup?, rtfUpperBound?, label? }
//
// `coreml: true` (parakeet, darwin-only) runs the entry against a copy of the
// GGUF staged next to its Core ML encoder sidecar under models/coreml/, so the
// engine's presence-driven sidecar loader picks it up for THIS entry only —
// plain cpu/metal entries keep resolving the sidecar-free models/ copy. The
// sidecar (`<stem>-encoder.mlmodelc`) is staged from the model manifest's
// `coremlSidecars` section by scripts/stage-integration-models.mjs; when it is
// absent (not yet published to the registry) the entry is skipped loudly.
//
...
// GGUF basename stems for the model types whose offline FastConformer encoder
// has a Core ML sidecar export (parakeet_coreml_path.cpp strips the quant tag,
// so one `<stem>-encoder.mlmodelc` serves every quant of the model).
const COREML_GGUF_STEMS = {
  tdt: 'parakeet-tdt-0.6b-v3',
  ctc: 'parakeet-ctc-0.6b'
}
...
// Error whose message marks a benign, pre-announced skip (missing Core ML
// sidecar) rather than a benchmark failure.
class SkipEntryError extends Error {}

// Stage a GGUF + Core ML sidecar pair under models/coreml/ for a
// `coreml: true` entry and return the extra env the benchmark needs:
// QVAC_TEST_GGUF_<TYPE> pins model resolution to the sidecar-adjacent copy
// (parakeet-helpers resolution order #2) and QVAC_PARAKEET_BENCHMARK_COREML
// makes the benchmark assert the sidecar actually loaded.
// `modelsDir` is injectable for tests only.
function prepareCoremlEntry (entry, modelsDir = MODELS_DIR) {
  ...
  // Mirrors the benchmark's desktop default quant (parakeet-helpers).
  ...
  // The engine resolves the sidecar relative to the GGUF it loads, so place a
  // link to the staged GGUF inside the sidecar directory's parent. A relative
  // symlink keeps the pair valid inside the CI model cache; fall back to a
  // hard link, then to a copy, for filesystems without symlink support.
...
function buildParakeetEnv (entry, label) {
  const env = {
    ...
    QVAC_PARAKEET_BENCHMARK_BACKEND: entry.backendHint
      ...
      : (normalizeBoolean(entry.coreml) ? 'coreml' : (process.env.QVAC_PARAKEET_BENCHMARK_BACKEND || '')),
...
function runBenchmarkEntry (pkgDir, entry, index) {
  ...
  const coremlEnv = engine === 'parakeet' && normalizeBoolean(entry.coreml)
    ? prepareCoremlEntry(entry)
    : null
...
function main () {
  ...
  console.log('')
  console.log(`Completed ${matrix.length - failures.length - skips.length}/${matrix.length} benchmark configuration(s).`)

  // Skips are pre-announced gaps (e.g. an unpublished Core ML sidecar), not
  // failures — but say so per entry, or the lane looks silently covered.
  if (skips.length > 0) {
...
module.exports = { SkipEntryError, prepareCoremlEntry, buildLabel, buildParakeetEnv }
