  ...
  async cancelIsolatesConcurrentBatches(
    ...
    const mkBatch = (tag: string, predict: number, ask: string) =>
      batchCompletion({
        modelId,
        prompts: [
          { id: `${tag}-a`, history: [{ role: 'user', content: ask }], generationParams: { temp: 0, seed: 1, predict } },
          { id: `${tag}-b`, history: [{ role: 'user', content: ask }], generationParams: { temp: 0, seed: 2, predict } }
        ]
      })

    // doomed generates long so it is still decoding when cancelled; survivor is short.
    const doomed = mkBatch('doomed', params.doomedPredict, 'Write a very long detailed story about an otter and a river.')
    ...
    if (doomedOutcome.resolved) {
      return { passed: false, output: 'doomed batch resolved, but its cancel should have rejected it' }
    }
    ...
    if (!Array.isArray(results) || results.length !== 2 || results.some((r) => r.final.contentText.length === 0)) {
      return { passed: false, output: `peer batch resolved but produced no content: ${JSON.stringify(results)}` }
    }

    return {
      passed: true,
      output: 'Per-group batch cancel: doomed batch cancelled, concurrent peer batch decoded to completion'
    }
  }
