...
#
# This republish advances the pin from 5bf602e7 to master 5e57a692 and raises the
# ggml-speech floor to 2026-08-07. engines/tts is byte-identical between the two
# commits -- every change in between lands in engines/audiogen -- so there is no
# engine change here; the move is for the source archive shared with the
# whisper-cpp / parakeet-cpp / audiogen-cpp ports, and for the ggml below.
#
# The ggml move carries substance. ggml PR #53 sizes the Vulkan matmul src0
# binding from nb[] instead of the view's elements counted as if packed, so a
# strided src0 view no longer addresses reads past the end of its descriptor --
# driver-dependent corruption reachable by any graph holding such a view -- and
# defaults prefer_host_memory on unified-memory adapters, which is every Android
# GPU this port targets. ggml PR #52 rewrites the OpenCL im2col on its default
# path; Parler's DAC and LavaSR are convolution-heavy, so that is the hot path
# for them on Adreno, and being a pure gather the rewrite is bit-exact.
#
...
#
# The 5bf602e7 pin (PR #123) added, relative to e91a2a27 before it, LavaSR's
# validated Android Mali Vulkan path and small-GEMM correctness workaround.
# Carries the fc844ce5 pin
