...
- **CosyVoice3** is trained on one instruction per synthesis, so engaging two
  controls throws rather than silently picking a winner.  `emotion: 'neutral'`
  and `pace: 'moderate'` engage nothing, which means they take the plain
  zero-shot path -- that path keeps the prompt speech tokens, where an
  instruction would drop them.  So neutral is the model's untouched natural
  voice rather than a fourth point on the same dial.
...
(reference transcript for the baked voice) and `streamLeftContextTokens`
(native chunk-streaming left context).  CosyVoice3 runs on **CPU** and emits
native **24 kHz**.
...
| `cosyvoice-tts.js` | CosyVoice3 batch synth with the cross-engine emotion option (24 kHz, CPU). `bare examples/cosyvoice-tts.js "Hello" happy` |
