...
// platform. 'coreml' is parakeet-only and not a ggml backend at all — it is
// the Apple Neural Engine encoder sidecar lane (darwin; the decoder stays on
// ggml), so it appears in parakeet's expected set but must not be reported as
// a whisper coverage gap (the whisper-cpp port builds without WHISPER_COREML).
