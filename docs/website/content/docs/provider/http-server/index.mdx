...
See [Installation](/sdk/installation) for environment-specific instructions of the SDK (e.g., Linux Vulkan runtime, Windows GPU drivers).
</Step>
...
Models are declared in `qvac.config.*` under the `serve.models` key. The server can only load models listed under this key — requests for unlisted models return 404. Each key in `serve.models` is a **model alias** — the name that HTTP clients use in the `model` field of their requests. For the full schema of `serve.models`, see [Configuration — `ServeConfig`](/sdk/configuration#serveconfig).

<Card href="/provider/http-server/connection" title="Connect AI tools">
...
- **`config`**: model config overrides passed to the underlying addon. Same options as [`modelConfig` in `loadModel()`](/sdk/reference/api#loadmodel).
...
<Card href="/provider/http-server/integration" title="Use @qvac/ai-sdk-provider">
...
| `reasoning_budget` | `reasoning_budget` | Boolean toggle for hybrid-thinking models: `true` keeps reasoning on, `false` disables it. Despite the name, it does not take a positive token cap — only the booleans (or their numeric equivalents `-1`/`0`). |

#### Reasoning (`reasoning_content`)

Models that emit `<think>…</think>` blocks (e.g. Qwen3.5) have their reasoning routed to the `reasoning_content` channel instead of leaking into `content`:

- **Blocking** — the reasoning is returned in `choices[0].message.reasoning_content`; `content` carries only the final answer.
- **Streaming** — reasoning arrives as `choices[0].delta.reasoning_content` chunks, interleaved with the regular `delta.content` chunks.

`reasoning_content` is present only when the model actually produced a reasoning block. Set `reasoning_budget: false` (see above) to disable reasoning entirely. Numeric `0` is also accepted; positive token caps are not.
...
<Callout type="warn">
On hybrid-thinking models, structured output with a tight `max_tokens` can hit `finish_reason: length` while generation is still inside the reasoning channel, leaving `content` empty. Disable reasoning (`reasoning_budget: false`) or raise `max_tokens` to avoid this.
</Callout>

#### Unsupported parameters

The following OpenAI parameters are accepted but ignored (a warning is logged): `n`, `logprobs`, `stop`, `top_logprobs`, `logit_bias`, `parallel_tool_calls`.

`stream_options.include_usage` is supported — see [Response: `finish_reason` and token usage](#response-finish_reason-and-token-usage).
...
`usage` is populated from the SDK's `CompletionStats`:

- **`prompt_tokens`** — real prompt token count when the SDK reports it, else `0`.
- **`completion_tokens`** — the SDK's `generatedTokens` when available, falling back to a whitespace word count of the output.
- **`total_tokens`** — the sum of the two.
- **`prompt_tokens_details.cached_tokens`** — present only when the SDK reports a KV-cache hit; it is the count of prompt tokens reused from a previous turn's cache, not the full prompt size.

Blocking and streaming share the same accounting, so counts do not drift between the two paths. By default a **streaming** response carries no `usage` object; set `stream_options: { include_usage: true }` to receive it as a trailing SSE chunk whose `choices` array is empty (it follows the `finish_reason` / `tool_calls` chunk), matching OpenAI's streaming shape.

<Callout type="info">
Real prompt-token accounting is currently **chat-only**. [`/v1/completions`](#legacy-completions) and [`/v1/responses`](#responses) still report `prompt_tokens` / `input_tokens` as `0`.
</Callout>
