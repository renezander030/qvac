...
In addition, you can add **custom plugins** — both your own and community ones — that extend QVAC's capabilities. In both cases, you'll use the QVAC configuration file and then bundle either with [QVAC CLI](/sdk/cli) or [programmatically via `@qvac/sdk/commands`](#notes).
...
| OCR | `@qvac/sdk/onnx-ocr/plugin` | [OCR](/sdk/ai-capabilities/ocr) |
...
[When developing for mobile environment, use the Expo CLI to prebuild your project only with the selected plugins:](/sdk/installation/#expo)
...
- See [QVAC CLI](/sdk/cli) to learn how to install and use it.
- When developing an Electron app, you can bundle the SDK during packaging via Electron Forge. See [Tutorials → Electron → Step 4: package for distribution](/sdk/tutorials/electron#step-4-package-for-distribution).
</Callout>
...
<Tab value="mobile" label="Expo">
[When developing for mobile environment, use the Expo CLI to prebuild your project with all listed plugins:](/sdk/installation/#expo)
