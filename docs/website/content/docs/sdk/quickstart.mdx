...
1. Provide a `process` global. Install `bare-process` and set it before using the SDK:
   ...
   ```
2. Register the plugins the example uses — Bare runs in-process and nothing auto-registers. See [Runtime registration on Bare](/sdk/configuration/plugins#runtime-registration-on-bare).
