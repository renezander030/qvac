...
Delegation is configured at model-load time by passing a `delegate` object to [`loadModel()`](/sdk/reference/api#loadmodel). The provider is started separately using [`startQVACProvider()`](/sdk/reference/api#startqvacprovider).

## Functions

**Provider:**
1. [`startQVACProvider()`](/sdk/reference/api#startqvacprovider) — bind the DHT server on the provider's keypair and start accepting delegated requests
2. [`stopQVACProvider()`](/sdk/reference/api#stopqvacprovider) — stop accepting requests

**Consumer:**
1. [`loadModel()`](/sdk/reference/api#loadmodel) — with `delegate` option
2. [`completion()`](/sdk/reference/api#completion) / [`transcribe()`](/sdk/reference/api#transcribe) / [`translate()`](/sdk/reference/api#translate) / etc. — same as local
...
- To stop a running provider, call [`stopQVACProvider()`](/sdk/reference/api#stopqvacprovider).
