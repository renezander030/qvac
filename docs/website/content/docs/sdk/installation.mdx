...
{/*
  The Python SDK is not launched yet, so its page is unpublished at
  content/_unpublished/python-sdk.mdx and its sidebar entry is removed from
  src/lib/custom-tree.ts. Restore both when it ships, along with this sentence:

  For Python, install `tetherto-qvac-sdk` from PyPI — see the Python SDK guide at /python-sdk

  The path is plain text rather than a markdown link because link-integrity
  regexes raw MDX and would flag a link to the unpublished page as broken.
*/}

### JS environments

- Node.js `>= v22.17`
- [Bare](https://bare.pears.com) `>= v1.24`
