...
**What it does:** `batchCompletion()` runs **multiple prompts through a single loaded model in one call**, instead of firing a separate [`completion()`](/sdk/reference/api#completion) call per prompt.
...
2. [`batchCompletion()`](/sdk/reference/api#batchcompletion)
...
You can load any [`llama.cpp`](https://github.com/ggml-org/llama.cpp)-compatible text-generation/chat model (same models as [Text generation](/sdk/ai-capabilities/text-generation)). Model file format: `*.gguf`.

- Per-prompt `tools`/`mcp` require a tool-capable model loaded with `modelConfig.tools: true`.
- Per-prompt `attachments` require a multimodal-capable model plus its matching `projectionModelSrc` — see [Multimodal](/sdk/ai-capabilities/multimodal).
...
* **Cancellable:** like any other request — see [Cancellation](/sdk/runtime/cancellation).
