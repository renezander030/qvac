...
Each stage maps to a QVAC capability — [Transcription](/sdk/ai-capabilities/transcription) (ASR), [Text generation](/sdk/ai-capabilities/text-generation) (LLM), and [Text-to-Speech](/sdk/ai-capabilities/text-to-speech) (TTS).
...
1. [`loadModel()`](/sdk/reference/api#loadmodel) three times — once per `modelType` (`"whisper"`, `"llm"`, `"tts"`).
2. [`transcribeStream()`](/sdk/reference/api#transcribestream) — open a streaming session that emits utterances on VAD-detected pauses.
3. [`completion()`](/sdk/reference/api#completion) — generate a response from the rolling conversation history (streamed).
4. [`textToSpeech()`](/sdk/reference/api#texttospeech) — synthesize the response into a PCM buffer.
5. [`unloadModel()`](/sdk/reference/api#unloadmodel) for each loaded model on shutdown.
...
- **Different voice:** replace the Supertonic constants with another TTS model (e.g. Chatterbox — see [Text-to-Speech](/sdk/ai-capabilities/text-to-speech)).
