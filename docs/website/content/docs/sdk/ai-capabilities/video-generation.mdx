...
2. [`video()`](/sdk/reference/api#video)
...
<Callout type="warn">
`width` and `height` must be positive multiples of **16**. Values that are multiples of 8 but not 16 (e.g. `264`, `520`) are rejected at the SDK boundary for both `txt2vid` and `img2vid`. **LTX-2** is stricter: `width` and `height` must be multiples of **32**, and `video_frames` must satisfy `8*k + 1` (e.g. `25`, `121`, `241`) with a maximum of **257** — validated against the loaded model before generation.
</Callout>
...
Two model families are supported — **WAN 2.1** and **LTX-2** — and the file layout is auto-selected at load time from the model sources you provide:
