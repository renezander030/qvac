...
Some models may be distributed as **multiple files** (shards) instead of a single large one. [`loadModel()`](/sdk/reference/api#loadmodel) supports sharded models and ensures that all shards are available before loading it. For this, shard file names **must** follow the pattern: `<name>-00001-of-0000X.<ext>`.
...
1. [`loadModel()`](/sdk/reference/api#loadmodel) — pass the path/URL of any shard; SDK fetches remaining shards
2. Use the model as usual ([`completion()`](/sdk/reference/api#completion), [`embed()`](/sdk/reference/api#embed), etc.)
