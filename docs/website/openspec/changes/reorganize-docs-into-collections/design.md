...
- **Tabs are derived from the tree.** When no explicit `tabs` array is passed, the layout calls `getSidebarTabs(tree)`, which walks the tree, collects every folder with `root: true`, and builds each tab from that folder — its `name` becomes the title, its `description` the subtitle, and its `index` (or first page) the landing URL. No separate tab list has to be declared or kept in sync with the tree.
...
- **`tabMode="navbar"` may not render as two distinct rows** → Spike it before any content moves. If the rendering is wrong, the fallback is a small custom bar reading the root folders off the same tree, which leaves every other decision intact.
